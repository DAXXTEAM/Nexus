...
@Nexus.on_message(filters.command(["pplay"], prefixes=["/", "."]))
async def play(_, message: Message):
    ...
    if video:
        ...
        try:
            results = PornhubApi(url, max_results=1).to_dict()            
        ...
        if len(message.command) < 2:
            await lel.edit(
                     "💌 **ᴜsᴀɢᴇ: /pplay ɢɪᴠᴇ ᴀ ᴛɪᴛʟᴇ sᴏɴɢ ᴛᴏ ᴘʟᴀʏ ᴍᴜsɪᴄ.**"
        ...
        try:
            results = PornhubApi(query, max_results=1).to_dict()
            url = f"https://es.pornhub.com{results[0]['url_suffix']}"            
            title = results[0]["title"]
            thumbnail = results[0]["thumbnails"][0]
            thumb_name = f"thumb{title}.jpg"
            thumb = requests.get(thumbnail, allow_redirects=True)
